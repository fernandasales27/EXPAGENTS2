...
export function createClassGroupController(service: ClassGroupService) {
  return async (request: IncomingMessage, response: ServerResponse): Promise<void> => {
    ...
    if (request.method === 'POST' && url.pathname === '/classes') {
      const body = JSON.parse(await readBody(request)) as { topicDescription: string; year: number; semester: number };
